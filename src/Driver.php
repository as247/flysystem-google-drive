...
class Driver implements DriverContract
{
	...
	protected function upload($path, $contents, Config $config=null)
	{
		...
		if (!$srcFile) {
			...
			if ($srcFile->getMimeType() === static::DIRMIME) {
		...
		if ($isResource) {
			...
			if ($mode === 'insert') {
				$request = $this->service->filesCreate($file);
			} else {
				$request = $this->service->filesUpdate($srcFile->getId(), $file);
			}
			...
			if ($mode === 'insert') {
				$obj = $this->service->filesCreate($file, $params);
			} else {
				$obj = $this->service->filesUpdate($srcFile->getId(), $file, $params);
			}
		}
